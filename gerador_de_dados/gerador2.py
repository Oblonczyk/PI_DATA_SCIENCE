...
def calcular_risco_evasao(aluno):
    ...
    desempenhos = [aluno[f'desempenho_{i}'] for i in range(1, 6)]
...
# Grades por combinação turma + semestre + bimestre
...
for aluno_id in range(1, num_alunos + 1):
    ...
    # Criar dicionário temporário para calcular risco
    aluno_dict = dict(zip([
...
# Salvar como CSV
df.to_csv('alunos_sem_erros.csv', index=False, encoding='utf-8-sig')
print("Arquivo CSV salvo como 'alunos_sem_erros.csv'")
